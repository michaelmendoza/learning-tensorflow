...
def plotOne(data):
    ...
    axs[0,0].plot(data.history['acc'])
    axs[0,0].plot(data.history['val_acc'])
    axs[0,0].set_title('With Data Aug')
    axs[0,0].set_ylabel('acc')
    axs[0,0].legend(["Train", "Test"], loc="lower right")

    axs[1,0].plot(data.history['loss'])
    axs[1,0].plot(data.history['val_loss'])
